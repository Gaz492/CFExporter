...
func readBuildJson(buildJsonDir string) buildJson {
	...
	if err != nil {
		pterm.Fatal.Println("Error reading .build.json:", err)
	}
...
func validateBuildFile() (err error) {
	BuildConfig = readBuildJson(*buildFile)
	if BuildConfig.PackAuthor == "" {
		return errors.New("invalid .build.json, Author not specified")
	} else if BuildConfig.MinecraftVersion == "" {
		return errors.New("invalid .build.json, Minecraft Version not specified")
	} else if BuildConfig.ModLoader == "" {
		return errors.New("invalid .build.json, Mod Loader not specified")
	} else if BuildConfig.ModLoaderVersion == "" {
		return errors.New("invalid .build.json, Mod Loader Version not specified")
	}
	return nil
}

func fsContains(s []fs.FileInfo, str string) bool {
	for _, v := range s {
		if v.Name() == str {
			return true
		}
